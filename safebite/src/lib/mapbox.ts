// Mapbox Directions helpers for fetching routes (with/without waypoint) as GeoJSON
...
// Requests Mapbox Directions for origin->destination with alternatives and GeoJSON geometry
...
// Request directions with an intermediate waypoint (origin -> waypoint -> destination)
// Requests Directions including a waypoint between origin and destination
export async function fetchDirectionsViaWaypoint(
  origin: Coordinate,
  waypoint: Coordinate,
  destination: Coordinate
): Promise<DirectionsRoute[]> {
  const profile = 'driving';
  const base = `https://api.mapbox.com/directions/v5/mapbox/${profile}/${origin[0]},${origin[1]};${waypoint[0]},${waypoint[1]};${destination[0]},${destination[1]}`;
  const url = `${base}?alternatives=true&geometries=geojson&overview=full&access_token=${CONFIG.MAPBOX_ACCESS_TOKEN}`;
  const res = await fetch(url);
  if (!res.ok) {
    throw new Error(`Directions request failed: ${res.status}`);
